...
function meeting_to_punch($item_id){
    ...
    if(!$meetingDuration){PodioItem::update($item_id, array(
        'fields'=>array('meeting-status-2' => 5),
        'hook'=>false));
    PodioComment::create('item',$item_id, array('value'=>'Please add the Meeting Duration before marking Completed.'));
    exit;
    }
    ...
    $billingCycleFilter = PodioItem::filter(4481866,array(
        "filters"=>array(
            'project'=>$projectItemID,
            'status-2'=>1))
    );
    ...
    foreach($attendeesArray as $attendeeID){
        ...
        $employeeDBItem = PodioItem::filter( 7099698, $attributes = array("filters"=>array('employee'=>$attendeeID)));
        $employeeDBItemID = $employeeDBItem[0]->item_id;

        $timecardItem = PodioItem::filter( 4177108, array("filters"=>array('employee-2'=>$employeeDBItemID,'status'=>1)));
        ...
        $punchFieldsArray = array(
            'employee-2' => $employeeDBItemID,
            'pay-period' => $timecardItemID,
            'billing-cycle' => (int)$billingCycleItemID,
            'action-item' => (int)$deliverableItemID,
            'deliverable' => (int)$milestoneItemID,
            'time-in-out' => array(
            ...
            'total-duration' => (int)$meetingDuration,
            'project' => (int)$projectItemID,
            'punch-billing-type' => $billingTypeID,
            'dashboard' => 406828105,
            'related-meeting' => (int)$item_id
        );



        PodioItem::create(4177143, array('fields' => $punchFieldsArray));
...
function pm_time_puncher($item_id, $revision_id){
    ...
    if($approvalStatusVal != 'Work Ready'){
        throw new Exception('Deliverable is not Work Ready. Could not Punch In.');
    }
    ...
    if($revisionToVal == 'Punch In') {
        ...
        if(!$milestoneItemID){
            PodioComment::create('item', $timeCardItemID, array('value' => "https://podio.com/users/".$triggerUserID."  Punch Error: No Milestone on Timecard"));
//HERE
            throw new Exception("https://podio.com/users/".$triggerUserID."  Punch Error: No Milestone on Timecard");
        }
...
function dev_time_puncher($item_id, $revision_id){
    ...
    if($revisionToVal == 'Punch In') {
        ...
        if(!$milestoneItemID){
            PodioComment::create('item', $timePuncherItemID, array('value' => "https://podio.com/users/".$triggerUserID."  Punch Error: No Milestone on Time Puncher"));

            throw new Exception("https://podio.com/users/".$triggerUserID."  Punch Error: No Milestone Type on Time Puncher");
        }
...
function marketing_time_puncher($item_id, $revision_id){
    ...
    $delivItemID = $item->item_id;

    $relatedProject = $item->fields['campaign']->values[0]->item_id;
    ...
    $timePuncher = PodioItem::filter( 15825433, $attributes = array("filters"=>array('employee'=>$employeeDBItemID)), $options = array() );

    $timePuncherItemID = $timePuncher[0]->item_id;
    ...
    $currentPunch = PodioItem::filter( 15825579, $attributes = array("filters"=>array('employee'=>$employeeDBItemID,'deliverable'=>$delivItemID,'status'=>'Working')), $options = array() );
...
try{
    ...
    switch ($automation){
        ...
        case 'card-cycle-generator':
            ...
            break;
        case 'card-cycle-generator-manual':
            $generate = card_cycle_generator_manual(date("Y-m-d"));
            break;
        case 'pm-time-clock':
        ...
        case 'dev-time-clock':
            ...
            break;
        case 'marketing-time-clock':
            $item_id = $requestParams['item_id'];
            $revision_id = $requestParams['item_revision_id'];
            marketing_time_puncher($item_id, $revision_id);
            break;
...
}catch(Exception $e)
{
    ...
    if($automation == 'pm-time-clock'){
        if($commentTrigger == "nope"){
            $e = "Incorrect Trigger, call ended";
        }
        elseif($commentTrigger == "yup") {
            PodioItem::update(
                $item_id,
                $attributes = array(
                    'fields' => array(
