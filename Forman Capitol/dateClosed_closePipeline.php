...
///PODIO ID VARIABLES
    ...
    $dateClosedFieldExID = 'closing-date';

//|||TECHeGO Step Break|||
//Step Item-#S57#
//Step 1 - Trigger / Update Action #

    $item = PodioItem::get($item_id);

    $dateClosed = $item->fields[$dateClosedFieldExID]->start;

    if($dateClosed){

        PodioItem::update($item_id, ['fields'=>[$dealStatusExID=>"Closed"]]);
